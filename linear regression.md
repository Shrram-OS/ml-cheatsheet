...
### Абстрактно / Магия
З.Ы. (Я знаю что постскриптум на то и пост что пишется после, но мне чёт похуй) читая разную литературу по ML я заметил что ОЧЕНЬ часто встречаются разные обозначения в  формулах, поэтому каждый смотрите обозначения. Я к тому что тут для вас на первый взгляд "другие" формулы, но они одни и те же если читать обозначения.
...
Квантуешь зависимость (binning) — когда точность не важна, важен диапазон. Бинирование - не категориальные признаки -> в категориальные признаки. Пример: bmi > 25 -> жирный уебок; bmi < 18 -> худой уебок; 25 < bmi < 18 - уебок.
...
Сжимаешь / растягиваешь (log / exp) — когда модель должна понимать проценты, а не штуки. Логарифмирование - `np.log1p()` если `scipy.stats.skew()` $>\approx$  0.7-0.8. Нужно для данных с большим хвостом, чтобы модель была устойчивее к выбросам. Примеры: заплаты, доход, кол-во пользователей, количество ссылок на сайты. Пример: разница между 100 и 500 грывень - пропасть, а между 1 000 100 и 1 000 500 - статистическая погрешность.
...
Метрика для оценки ошибок.
#### Абстрактно / Магия
Формула которая помогает оценивать точность модели.

Устойчива к выбросам. Чем меньше тем лучше.  
#### Под капотом
$$\rm{MAE} = \frac{1}{n}\sum_{n}^{i=1}|\hat y_i - y_i|$$
...
По формуле понятно что она учитывает абсолютные отклонения и боле устойчива к выбросам чем остальные метрики.

### MSE, RMSE
$\rm{MSE}$ - Mean Squared Error - Средняя квадратическая ошибка

$\rm{RMSE}$ - Root Mean Squared Error - Корень из MSE

Нахуя корень? Потому что во-первых, так мы возвращаем к размерности данным, а во-вторых, она более человеко-читаемая.
#### Абстрактно / Магия
Формула которая помогает оценивать точность модели.

Сильно штрафует за большие ошибки. Чем меньше тем лучше. 
#### Под капотом


$\rm{MSE} = \frac{1}{n}\sum_{n}^{i=1}(\hat y_i - y_i)^2$

-  $\hat y_i$ - Предикт
- $y_i$ - Таргет
- $n$ - Количество рядов 

$$\rm{RMSE} = \sqrt{\frac{1}{n}\sum_{n}^{i=1}(\hat y_i - y_i)^2}$$
...
### MAPE
$\rm{MAPE}$ - Средняя абсолютная процентная ошибка.
#### Абстрактно / Магия
Формула которая помогает оценивать точность модели.

Показывает ошибку в процентах. Особенно полезну чтобы заказчику дегенерату показать "точность".

#### Под капотом
$$\text{MAPE} = \frac{1}{n} \sum_{n}^{i=1} \left| \frac{y_i - \hat y_i}{y_i} \right| \times 100\%$$

-  $\hat y_i$ - Предикт
- $y_i$ - Таргет
- $n$ - Количество рядов 

Конечно же каждый не долбаеб заметил что мы тут можем поделить на 0, но и те кто писали все библиотеки тоже не долбаебы и заменяют там 0 на $\epsilon$ (epsilon) $\approx$ $2.22 \times 10^{-16}$. Поэтому вы просто проебали время читая этот абзац ещё и обдумывая его.

### $R^2$
$R^2$ - Коэффициент детерминации
#### Абстрактно / Магия
Формула которая помогает оценивать качество модели.

Как на него смотреть?
- $R^2 = 1$ - идеальная модель
- $R^2 = 0$ - модель на уровне среднего значения
- $R^2 < 0$ - АХХПАХП НУ ТЫ ВАЩЕ ЕБЛАН. (Модель конченная, переделывай)
...
- $\hat y_i$ - предикт
- $y_i$ - таргет
- $\bar y$ - среднее арифметическое всех реальных значений
### Кастомные метрики
Зависит от задачи. Напомню что вы ебаные пешки бизнеса.

Клиенту может быть гараздо важнее не переоценивать, а недооценивать, или наоборот. Или клиенту гараздо важнее не переоценивать больше чем на 20к, но недооценивать можно хоть на 100к. Писать кастомные метрики это норма. [Как обучить модель учитывать такие нюансы (Sample weighting)](linear%20regression.md#sample-weighting)
